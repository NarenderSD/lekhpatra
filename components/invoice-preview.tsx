...
export default function InvoicePreview({
  ...
  return (
    <div className="bg-white text-black p-8 min-h-[29.7cm] w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 mb-8">
        <div>
          ...
          <div className="text-black/70">
            ...
            {invoiceData.fromMobile && <p>Mobile: {invoiceData.fromMobile}</p>}
          </div>
        </div>
        <div className="mt-4 sm:mt-0">
          ...
          <div className="text-black/70">
            ...
            {invoiceData.toMobile && <p>Mobile: {invoiceData.toMobile}</p>}
          </div>
        </div>
      </div>

      {/* New: Payment Details in Preview */}
      {(invoiceData.bankName || invoiceData.accountNumber || invoiceData.ifscCode || invoiceData.swiftBic) && (
        <div className="mb-8">
          <h2 className="text-lg font-semibold mb-2">Payment Details:</h2>
          <div className="text-black/70">
            {invoiceData.bankName && <p>Bank Name: {invoiceData.bankName}</p>}
            {invoiceData.accountNumber && <p>Account Number: {invoiceData.accountNumber}</p>}
            {invoiceData.ifscCode && <p>IFSC Code: {invoiceData.ifscCode}</p>}
            {invoiceData.swiftBic && <p>SWIFT/BIC: {invoiceData.swiftBic}</p>}
          </div>
        </div>
      )}
      ...
      {/* New: Terms and Conditions in Preview */}
      {invoiceData.termsAndConditions && (
        <div className="mb-8">
          <h2 className="text-lg font-semibold mb-2">Terms and Conditions:</h2>
          <p className="text-black/70 whitespace-pre-line">{invoiceData.termsAndConditions}</p>
        </div>
      )}
